...
import * as jobDuck from "../../store/ducks/user.duck";
function Dashboard(props) {
  const [allNurseinfo,setAllNurseinfo] = React.useState([]);
  const [allClientinfo,setAllClientinfo] = React.useState([]);
  const [allJobinfo,setAllJobinfo] = React.useState([]);
  const [earning,setEarning] = React.useState(0);
  useEffect(() => {
    setAllNurseinfo(props.nurses);
    setAllClientinfo(props.clients);
    setAllJobinfo(props.jobs);
    var temp = props.charges;
    var tempAmount = 0;
    if(props.charges){
         temp.forEach(element => {
      tempAmount += element.amount /100;
    });
    setEarning(tempAmount)
    }
...
export default connect(
  mapStateToProps,
  {...authDuck.actions, ...userDuck.actions, ...jobDuck.actions}
)(Dashboard)
